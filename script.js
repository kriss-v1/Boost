...
const INSTAGRAM_USERNAME = "kizboost";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const kizAiButton =
        document.getElementById('kizAiButton');

    const kizAiWindow =
        document.getElementById('kizAiWindow');

    const kizAiClose =
        document.getElementById('kizAiClose');

    const kizAiInput =
        document.getElementById('kizAiInput');

    const kizAiSend =
        document.getElementById('kizAiSend');

    const kizAiMessages =
        document.getElementById('kizAiMessages');
    ...
    if (
        ...
        // Open KIZ AI
        kizAiButton.addEventListener('click', () => {
            kizAiWindow.hidden = false;
            kizAiInput.focus();
        });

        // Close KIZ AI
        kizAiClose.addEventListener('click', () => {
            kizAiWindow.hidden = true;
        });

        // Send button
        kizAiSend.addEventListener('click', sendKizAiMessage);

        // Enter key
        kizAiInput.addEventListener('keydown', (event) => {
            if (event.key === 'Enter') {
        ...
        async function sendKizAiMessage() {

            const message = kizAiInput.value.trim();

            if (!message) return;

            // Show user's message
            addKizAiMessage(message, 'user');

            kizAiInput.value = '';

            kizAiInput.disabled = true;
            kizAiSend.disabled = true;
            ...
            kizAiMessages.appendChild(typingMessage);
            ...
            try {
                ...
                const typing =
                    document.getElementById('kizAiTyping');
                ...
                addKizAiMessage(
                    'Sorry, I could not connect to KIZ AI right now. Please try again.',
                    'ai'
                );

            } finally {

                kizAiInput.disabled = false;
                kizAiSend.disabled = false;

                kizAiInput.focus();
            }
        }

        function addKizAiMessage(
            message,
            type
        ) {
        ...
        function scrollKizAiToBottom() {

            kizAiMessages.scrollTop =
                kizAiMessages.scrollHeight;
        }
    }
});